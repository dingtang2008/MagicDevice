...
package com.elvis.magicdevice.RootTools.internal;
...
import com.elvis.magicdevice.RootShell.RootShell;
import com.elvis.magicdevice.RootShell.execution.Command;
import com.elvis.magicdevice.RootShell.execution.Shell;
import com.elvis.magicdevice.RootTools.Constants;
import com.elvis.magicdevice.RootTools.RootTools;
import com.elvis.magicdevice.RootTools.containers.Mount;
import com.elvis.magicdevice.RootTools.containers.Permissions;
import com.elvis.magicdevice.RootTools.containers.Symlink;
...
public final class RootToolsInternalMethods {
		...
		try {
			...
			if (checkUtil("cp")) {
				...
				if (preserveFileAttributes) {
					command = new Command(0, false, "cp -fp " + source + " "
							+ destination);
					...
					command = new Command(0, false, "cp -f " + source + " "
							+ destination);
					Shell.startRootShell().add(command);
					commandWait(Shell.startRootShell(), command);
				...
				if (checkUtil("busybox") && hasUtil("cp", "busybox")) {
					...
					if (preserveFileAttributes) {
						command = new Command(0, false, "busybox cp -fp "
								+ source + " " + destination);
						Shell.startRootShell().add(command);
						commandWait(Shell.startRootShell(), command);

					} else {
						command = new Command(0, false, "busybox cp -f "
								+ source + " " + destination);
						Shell.startRootShell().add(command);
						commandWait(Shell.startRootShell(), command);
							...
							// if cat is available and has appropriate
							// permissions
					if (checkUtil("cat")) {
						...
						// copy with cat
						command = new Command(0, false, "cat " + source + " > "
								+ destination);
						Shell.startRootShell().add(command);
						commandWait(Shell.startRootShell(), command);
						...
						if (preserveFileAttributes) {
							...
							command = new Command(0, false, "chmod "
		...
		try {
			...
			if (hasUtil("rm", "toolbox")) {
				...
				if (checkUtil("busybox") && hasUtil("rm", "busybox")) {
					...
					Command command = new Command(0, false, "busybox rm -rf "
							+ target);
					Shell.startRootShell().add(command);
					commandWait(Shell.startRootShell(), command);
	...
	public void fixUtil(String util, String utilPath) {
		try {
			...
			if (foundPaths.size() > 0) {
				for (String path : foundPaths) {
					Command command = new Command(0, false, utilPath + " rm "
							+ path + "/" + util);
				...
				Command command = new Command(0, false, utilPath + " ln -s "
						+ utilPath + " /system/bin/" + util, utilPath
						+ " chmod 0755 /system/bin/" + util);
				RootShell.getShell(true).add(command);
				commandWait(RootShell.getShell(true), command);
	...
	public List<String> getBusyBoxApplets(String path) throws Exception {
		...
		Command command = new Command(Constants.BBA, false, path
				+ "busybox --list") {
	...
	public String getBusyBoxVersion(String path) {
		...
		try {
			Command command = new Command(Constants.BBV, false, path
					+ "busybox") {
				@Override
				public void commandOutput(int id, String line) {
					line = line.trim();
	...
	public String getInode(String file) {
		try {
			Command command = new Command(Constants.GI, false,
	...
	public Permissions getFilePermissionsSymlinks(String file) {
		...
		if (RootTools.exists(file)) {
			...
			try {
				...
				Command command = new Command(Constants.FPS, false, "ls -l "
						+ file, "busybox ls -l " + file,
	...
	public long getSpace(String path) {
		...
		try {
			final Command command = new Command(Constants.GS, false, "df "
					+ path) {
	...
	public String getSymlink(String file) {
		...
		try {
			...
			Command command = new Command(Constants.GSYM, false, "ls -l "
					+ file) {
	...
	public ArrayList<Symlink> getSymlinks(String path) throws Exception {
		...
		command = new Command(0, false, "find " + path
	...
	/**
	 * This will launch the Play Store looking for SuperSU, but will return the
	 * intent fired and starts the activity with startActivityForResult
	 *
	 * @param activity
	 *            pass in your Activity
	 * @param requestCode
	...
	private void commandWait(Shell shell, Command cmd) throws Exception {
		...
		while (!cmd.isFinished()) {
			...
			RootTools.log(Constants.TAG, "Processed "
					+ cmd.totalOutputProcessed + " of " + cmd.totalOutput
					+ " output from command.");
