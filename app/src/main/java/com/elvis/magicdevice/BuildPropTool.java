...
public class BuildPropTool{
	 ...
	 * @param useRoot
	 *            If set to true, root will be required and the values retrieved will be up to date.
	...
	private static void transferFileToSystem(){
		...
		try{
			...
			os.writeBytes("mv -f /system/build.prop " + "/sdcard/build.prop.bak\n");
	...
	private static void createTempFile(){
		...
		try{
			RootTools.remount("/system/", "rw");
			RootTools.copyFile("/system/build.prop", "/sdcard/buildprop.tmp", false, true);
			process = Runtime.getRuntime().exec("su");
			os = new DataOutputStream(process.getOutputStream());
			os.writeBytes("chmod 777 " + "/sdcard/buildprop.tmp\n");
			RootTools.remount("/system/", "ro");
			os.writeBytes("exit\n");
			os.flush();
			process.waitFor();
